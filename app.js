(function(){
  ...
  let shipProgress=0,lastFrame=0,lastRiskCheck=0,lowSpeedSeconds=0,certificateReady=false,qualifiedZone='',qualifiedSpeed=0;
  ...
  certificateButton.addEventListener('click',()=>{if(!certificateReady)return;const now=new Date(),certificateCode=`NF-DEMO-${now.getFullYear()}-${String(now.getTime()).slice(-6)}`;document.getElementById('certificateZone').textContent=qualifiedZone;document.getElementById('certificateSpeed').textContent=`${qualifiedSpeed} kn 이하 · 기준 ${CERT_DISPLAY_MINUTES}분 유지 (웹 시연 ${CERT_REQUIRED}초)`;document.getElementById('certificateDate').textContent=new Intl.DateTimeFormat('ko-KR',{dateStyle:'long',timeStyle:'short'}).format(now);document.getElementById('certificateNumber').textContent=certificateCode;document.getElementById('certificateClaimNumber').value=certificateCode;certificateDialog.showModal();});
  certificateReset.addEventListener('click',()=>{certificateReady=false;lowSpeedSeconds=0;qualifiedZone='';qualifiedSpeed=0;certificateButton.disabled=true;certificateButton.classList.remove('ready');certificateProgress.style.width='0%';certificateStatus.textContent=`위험구역에서 8kn 이하 ${CERT_DISPLAY_MINUTES}분 감속`;if(certificateDialog.open)certificateDialog.close();showToast('감속 확인 진행을 초기화했습니다.');});
